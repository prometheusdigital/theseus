<?php
/**
 * The Theseus Widgets API extends the WordPress Widgets API. Where WordPress uses 'sidebar', Theseus uses
 ...
 *
 * @package PrometheusDigital\Theseus\API\Widgets
 *
 * @since 1.0.0
 */
 ...
 *
 * Since a Theseus widget area is using the WordPress sidebar, this function registers a WordPress sidebar using
 ...
 * supported. Theseus widgets are built using the Theseus HTML API which allows full control over HTML markup
 * and attributes.
 *
 ...
 */
function beans_register_widget_area( $args = [] ) {
	 ...
	 * @since 1.0.0
	 */
	$defaults = apply_filters( 'beans_widgets_area_default_args', [
		'beans_type'                 => 'stack',
 ...
 *
 * Since a Theseus widget area is using the WordPress sidebar, this function deregisters the defined
 ...
 *
 * Since a Theseus widget area is using the WordPress sidebar, this function checks if the defined sidebar
...
function beans_setup_widget() {
	...
	$id = beans_get( $_beans_widget_area['current_widget'], $widgets );
...
function _beans_setup_widgets( $widget_area_content ) {
	...
	foreach ( explode( '<!--widget-end-->', $widget_area_content ) as $content ) {
		...
		$data = beans_get( $id, $wp_registered_widgets );
		...
		foreach ( [ 'id', 'name', 'classname', 'description' ] as $var ) {
		...
		$_beans_widgets[ $widget['id'] ] = array_merge( $widget, [
