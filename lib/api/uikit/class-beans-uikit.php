 ...
 *
 * @package PrometheusDigital\Theseus\API\UIkit
 *
 * @since   1.0.0
 */
...
final class _Beans_Uikit {
	 ...
	 * @var array
	 */
	private $ignored_components = [ 'uikit-customizer', 'uikit' ];
	...
	public function register_less_components() {
		$components = $this->get_registered_component_paths( [ 'variables' ] );
		...
		// Add fixes.
		$components[] = THESEUS_API_PATH . 'uikit/src/fixes.less';
	...
	public function register_js_components() {
		return $this->get_registered_component_paths( [ 'core', 'utility', 'touch' ], false );
	}
	...
	public function get_less_directories( $type ) {
		...
		$directories = [ THESEUS_API_PATH . 'uikit/src/less/' . $type ];
	...
	public function get_js_directories( $type ) {
		...
		// Define the UIkit src directory.
		return [ THESEUS_API_PATH . 'uikit/src/js/' . $type ];
	}
	...
	public function get_all_components( $type ) {
		...
		foreach ( $directories as $dir_path ) {
			...
			$component_names = array_map( [ $this, 'to_filename' ], $this->get_all_files( $dir_path ) );
	...
	public function get_autoload_components( array $components ) {
		...
		foreach ( (array) $components as $component ) {
			$component_dependencies = beans_get( $component, self::$configured_components_dependencies, [] );
