...
final class _Beans_Page_Compiler {
	 ...
	 * @var array
	 */
	private $handles_not_to_compile = [ 'admin-bar', 'open-sans', 'dashicons' ];
	...
	public function init() {
		add_action( 'wp_enqueue_scripts', [ $this, 'compile_page_styles' ], 9999 );
		add_action( 'wp_enqueue_scripts', [ $this, 'compile_page_scripts' ], 9999 );
	}
	...
	public function compile_page_styles() {
		...
		beans_compile_css_fragments( 'beans', $styles, [ 'version' => null ] );
	}
	...
	public function compile_page_scripts() {
		...
		add_action( 'wp_print_scripts', [ $this, 'dequeue_scripts' ], 9999 );

		beans_compile_js_fragments( 'beans', $scripts, [
	...
	private function maybe_add_media_query_to_src( $asset ) {
		...
		$asset->src = add_query_arg( [ 'beans_compiler_media_query' => $asset->args ], $asset->src );
	}
