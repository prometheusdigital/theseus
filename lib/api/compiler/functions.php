 ...
 *
 * The Theseus Compiler compiles multiple internal or external CSS, LESS and JS files on a
 ...
 * @return void|bool
 */
function beans_compile_css_fragments( $id, $fragments, $args = [] ) {
 ...
 * @return void|bool
 */
function beans_compile_less_fragments( $id, $fragments, $args = [] ) {
 ...
 * @return void|bool
 */
function beans_compile_js_fragments( $id, $fragments, $args = [] ) {
 ...
 * @return void
 */
function _beans_compile_fragments( $id, $format, $fragments, array $args = [], $is_script = false ) {
	$config = [
		'id'        => $id,
...
function beans_flush_compiler( $id, $file_format = false, $admin = false ) {
	...
	// Always flush Theseus' global cache.
	if ( ! $beans_flushed ) {
		$beans_flushed = true;
...
/**
 * Get absolute path to the Theseus' compiler directory.
 *
 * @since 1.0.0
 *
...
function beans_get_compiler_dir( $is_admin = false ) {
	...
	/**
	 * Deprecated. Filter the Theseus compiler directory.
	 *
...
/**
 * Get absolute URL to the Theseus' compiler directory.
 *
 * @since 1.3.0
 *
 * @param bool $is_admin Optional. When true, gets the admin compiler directory. Default is false.
...
/**
 * Add the "compiler options" to the Theseus Settings page.
 *
 * @since 1.5.0
 *
...
function _beans_is_compiler_dev_mode() {
	...
	if ( defined( 'THESEUS_COMPILER_DEV_MODE' ) ) {
		return THESEUS_COMPILER_DEV_MODE;
	}
