...
final class _Beans_Compiler {
	...
	public function run_compiler() {
		...
		add_filter( 'filesystem_method', [ $this, 'modify_filesystem_method' ] );
		...
		remove_filter( 'filesystem_method', [ $this, 'modify_filesystem_method' ] );
	}
	...
	public function format_content( $content ) {
		...
		if ( 'style' === $this->config['type'] ) {
			...
			if ( 'less' === $this->config['format'] ) {
				...
				if ( ! class_exists( 'Beans_Lessc' ) ) {
					require_once THESEUS_API_PATH . 'compiler/vendors/lessc.php';
				}
		...
		if ( 'script' === $this->config['type'] && ! _beans_is_compiler_dev_mode() && $this->config['minify_js'] ) {
			...
			if ( ! class_exists( 'JSMin' ) ) {
				require_once THESEUS_API_PATH . 'compiler/vendors/js-minifier.php';
			}
	...
	private function kill() {
		...
		$html = beans_output( 'beans_compiler_error_title_text', sprintf(
			'<h2>%s</h2>',
			__( 'Not cool, Theseus cannot work its magic :(', 'tm-beans' )
		) );

		$html .= beans_output( 'beans_compiler_error_message_text', sprintf(
			'<p>%s</p>',
			__( 'Your current install or file permission prevents Theseus from working its magic. Please get in touch with Theseus support. We will gladly get you started within 24 - 48 hours (working days).', 'tm-beans' )
		) );

		$html .= beans_output( 'beans_compiler_error_contact_text', sprintf(
			'<a class="button" href="https://prometheus-digital.com/theseus/contact/?compiler_report=1" target="_blanc">%s</a>',
			__( 'Contact Theseus Support', 'tm-beans' )
		) );
