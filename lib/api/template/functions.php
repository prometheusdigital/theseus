<?php
/**
 * The Theseus Templates API allows to load Theseus template files as well as loading the entire document.
 *
 * @package PrometheusDigital\Theseus\API\Template
 *
 * @since   1.0.0
 */

/**
 * Load and render the entire document (web page).  This function is the root of the Theseus' framework hierarchy.
 * Therefore, when calling it, Theseus runs, building the web page's HTML markup and rendering it out to the
 * browser.
 *
...
function beans_load_document() {
	 ...
	 *
	 * This hook is the root of Theseus's framework hierarchy. It all starts here!
	 *
	 * @since 1.0.0
	 */
 ...
 *
 * This function loads Theseus's default template file. It must be called from a secondary template file
...
function beans_load_default_template( $file ) {
	$file = THESEUS_STRUCTURE_PATH . basename( $file );
...
function beans_load_fragment_file( $fragment ) {
	...
	if ( ! file_exists( THESEUS_FRAGMENTS_PATH . $fragment . '.php' ) ) {
		return false;
	}

	require_once THESEUS_FRAGMENTS_PATH . $fragment . '.php';
