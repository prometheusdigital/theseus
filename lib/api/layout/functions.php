<?php
/**
 * The Theseus Layout API controls what and how Theseus main section elements are displayed.
 *
 * Layouts are:
 *      - "c" - content only
 ...
 *
 * @package PrometheusDigital\Theseus\API\Layout
 *
 * @since   1.5.0
 */
...
function beans_get_layout_class( $id ) {
	 ...
	 * }
	 */
	$args = apply_filters( 'beans_layout_grid_settings', [
		'grid'              => 4,
		'sidebar_primary'   => 1,
		'sidebar_secondary' => 1,
		'breakpoint'        => 'medium',
...
/**
 * Generate layout elements used by Theseus 'imageradio' option type.
 *
 ...
 *
 * @return array Layouts ready for Theseus 'imageradio' option type.
 */
function beans_get_layouts_for_options( $add_default = false ) {
	$base    = THESEUS_ADMIN_ASSETS_URL . 'images/layouts/';
	$layouts = [
		'c' => [
			'src'                => $base . 'c.png',
...
function beans_has_primary_sidebar( $layout ) {
	...
	if ( ! in_array( $layout, [ 'c_sp', 'sp_c', 'c_sp_ss', 'sp_c_ss', 'sp_ss_c' ], true ) ) {
		return false;
	}
...
function beans_has_secondary_sidebar( $layout ) {
	...
	if ( ! in_array( $layout, [ 'c_ss', 'ss_c', 'c_sp_ss', 'sp_c_ss', 'sp_ss_c' ], true ) ) {
		return false;
	}
