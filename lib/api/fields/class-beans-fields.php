<?php
/**
 * Handles standardizing and registering the fields into the Theseus Fields' Container.
 *
 * @package PrometheusDigital\Theseus\API\Fields
 *
 * @since   1.0.0
 */

/**
 * The Theseus Fields' Container.
 *
 * @since   1.0.0
 * @ignore
 * @access  private
...
final class _Beans_Fields {
	 ...
	 * @var array
	 */
	private static $field_types_loaded = [];
	 ...
	 * @var array
	 */
	private static $field_assets_hook_loaded = [];
	...
	public function register( array $fields, $context, $section ) {
		...
		add_action( 'admin_enqueue_scripts', [ $this, 'load_fields_assets_hook' ] );
		add_action( 'customize_controls_enqueue_scripts', [ $this, 'load_fields_assets_hook' ] );
	...
	private function do_once() {
		...
		add_action( 'customize_controls_enqueue_scripts', [ $this, 'enqueue_global_assets' ] );

		// Load the field label and description handler.
		require_once THESEUS_API_PATH . 'fields/types/field.php';
	...
	private function load_fields() {
		...
		foreach ( $this->field_types as $type ) {
			...
			$path = THESEUS_API_PATH . "fields/types/{$type}.php";
	...
	public function enqueue_global_assets() {
		$css = THESEUS_API_URL . 'fields/assets/css/fields' . THESEUS_MIN_CSS . '.css';
		$js  = THESEUS_API_URL . 'fields/assets/js/fields' . THESEUS_MIN_CSS . '.js';

		wp_enqueue_style( 'beans-fields', $css, false, THESEUS_VERSION );
		wp_enqueue_script( 'beans-fields', $js, [ 'jquery' ], THESEUS_VERSION );
