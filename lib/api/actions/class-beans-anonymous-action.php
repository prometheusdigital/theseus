<?php
/**
 * This class creates an anonymous callback, which is required since Theseus still supports PHP 5.2.
 *
...
final class _Beans_Anonymous_Action {
	...
	public function __construct( $hook, array $callback, $priority = 10, $number_args = 1 ) {
		...
		add_action( $hook, [ $this, 'callback' ], $priority, $number_args );
	}
	...
	public function callback() {
		echo call_user_func_array( $this->callback[0], $this->callback[1] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- The callback handles escaping its output, as Theseus does not know what HTML or content will be passed back to it.
	}
}
