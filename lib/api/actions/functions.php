<?php
/**
 * Theseus Actions extends WordPress Actions by registering each action with a unique ID.
 *
 * While WordPress requires two or three arguments to remove an action, Theseus
 * actions can be modified, replaced, removed or reset using only the ID as a reference.
 *
 * @package PrometheusDigital\Theseus\API\Actions
 *
 * @since   1.5.0
 */
 ...
 * with the exception of being registered by ID within Theseus in order to be manipulated by the other Theseus
 * Actions functions.
 *
 * @since 1.0.0
 * @since 1.5.0 Returns false when action is not added via add_action.
 *
 * @param string   $id       The action's Theseus ID, a unique ID tracked within Theseus for this action.
 ...
 *
 * This function is a shortcut of {@see beans_add_action()}. It does't require a Theseus ID as it uses the
...
function beans_modify_action( $id, $hook = null, $callback = null, $priority = null, $args = null ) {
	...
	// Merge the modified parameters and register with Theseus.
 ...
 * registered with Theseus actions for the given ID and status. Else, returns true when complete.
 *
 * @since  1.0.0
 * @ignore
 * @access private
 ...
 *
 * If the action's configuration has not already been registered with Theseus, just store it.
 *
 * @since  1.0.0
 * @ignore
 * @access private
...
function _beans_merge_action( $id, array $action, $status ) {
	...
	// If the action's configuration is already registered with Theseus, merge the new configuration with it.
	if ( ! empty( $registered_action ) ) {
...
function _beans_build_action_array( $hook = null, $callback = null, $priority = null, $args = null ) {
	...
	foreach ( [ 'priority', 'args' ] as $arg_name ) {
		$arg = ${$arg_name};
...
function _beans_add_anonymous_action( $hook, array $callback, $priority = 10, $number_args = 1 ) {
	require_once THESEUS_API_PATH . 'actions/class-beans-anonymous-action.php';
