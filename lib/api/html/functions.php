<?php
/**
 * The Theseus HTML component contains a powerful set of functions to create flexible and easy overwritable HTML markup,
 * attributes and content.
 *
 * @package PrometheusDigital\Theseus\API\HTML
 *
 * @since   1.5.0
 */
 ...
 * @return string|void
 */
function beans_open_markup( $id, $tag, $attributes = [] ) {
	$args            = func_get_args();
 ...
 * @return void
 */
function beans_open_markup_e( $id, $tag, $attributes = [] ) {
	$args = func_get_args();
 ...
 * @return string|void
 */
function beans_selfclose_markup( $id, $tag, $attributes = [] ) {
	global $_beans_is_selfclose_markup;
 ...
 * @return void
 */
function beans_selfclose_markup_e( $id, $tag, $attributes = [] ) {
	$args = func_get_args();
...
/**
 * Register the wrap markup with Theseus using the given markup ID.
 *
 ...
 * @return bool
 */
function beans_wrap_markup( $id, $new_id, $tag, $attributes = [] ) {
	...
	_beans_add_anonymous_action( $id . '_before_markup', [ 'beans_open_markup', $args ], 9999 );

	unset( $args[3] );

	_beans_add_anonymous_action( $id . '_after_markup', [ 'beans_close_markup', $args ], 1 );

	return true;
}

/**
 * Register the wrap inner content's markup with Theseus using the given markup ID.
 *
 * This function registers an anonymous callback to the following action hooks:
 *
 ...
 * @return bool
 */
function beans_wrap_inner_markup( $id, $new_id, $tag, $attributes = [] ) {
	...
	_beans_add_anonymous_action( $id . '_prepend_markup', [ 'beans_open_markup', $args ], 1 );

	unset( $args[3] );

	_beans_add_anonymous_action( $id . '_append_markup', [ 'beans_close_markup', $args ], 9999 );
 ...
 *
 * The attributes are registered in Theseus via the given ID.  Using this ID, we can hook into the filter, i.e.
 ...
 */
function beans_add_attributes( $id, $attributes = [] ) {
	$args    = func_get_args();
...
function _beans_is_html_dev_mode() {
	...
	if ( defined( 'THESEUS_HTML_DEV_MODE' ) ) {
		return (bool) THESEUS_HTML_DEV_MODE;
	}
