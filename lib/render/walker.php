...
class _Beans_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = [] ) {
		...
		if ( $depth > 0 || in_array( $type, [ 'sidenav', 'offcanvas' ], true ) ) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
		...
		if ( isset( $child_indicator ) ) {
			$item_output .= beans_open_markup( "beans_menu_item_child_indicator[_{$item_id}]", 'i', [ 'class' => 'uk-icon-caret-down uk-margin-small-left' ], $item, $depth, $args );
