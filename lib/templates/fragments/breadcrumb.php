...
function beans_breadcrumb() {
	...
	// Custom post type.
	if ( ! in_array( $post_type, [ 'page', 'attachment', 'post' ], true ) && ! is_404() ) {
	...
	// Open breadcrumb.
	beans_open_markup_e( 'beans_breadcrumb', 'ul', [ 'class' => 'uk-breadcrumb uk-width-1-1' ] );
		$i = 0;
	...
	foreach ( $breadcrumbs as $breadcrumb_url => $breadcrumb ) {
		...
		if ( count( $breadcrumbs ) - 1 !== $i ) {
			beans_open_markup_e( 'beans_breadcrumb_item', 'li' );
				...
				beans_open_markup_e( 'beans_breadcrumb_item_link', 'a', [ 'href' => $breadcrumb_url ] ); // Automatically escaped.
			...
			beans_open_markup_e( 'beans_breadcrumb_item[_active]', 'li', [ 'class' => 'uk-active uk-text-muted' ] );
