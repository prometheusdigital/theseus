...
function beans_post_search_title() {
	...
	beans_open_markup_e( 'beans_search_title', 'h1', [ 'class' => 'uk-article-title' ] );

		printf( '%1$s%2$s', beans_output( 'beans_search_title_text', esc_html__( 'Search results for: ', 'tm-beans' ) ), get_search_query() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Each placeholder is escaped.
...
function beans_post_meta() {
	...
	beans_open_markup_e( 'beans_post_meta', 'ul', [ 'class' => 'uk-article-meta uk-subnav uk-subnav-line' ] );
...
function beans_post_image() {
	...
	/**
	 * Filter whether Theseus should handle the image edition (resize) or let WP do so.
	 *
	 * @since 1.2.5
	 *
	 * @param bool $edit True to use Theseus Image API to handle the image edition (resize), false to let {@link https://codex.wordpress.org/Function_Reference/the_post_thumbnail the_post_thumbnail()} taking care of it. Default true.
	 */
	...
	beans_open_markup_e( 'beans_post_image', 'div', [ 'class' => 'tm-article-image' ] );
	...
	if ( $edit ) {
		...
		// Theseus API isn't available, use wp_get_attachment_image_attributes filter instead.
		the_post_thumbnail();
	}
...
function beans_post_content_navigation() {
	echo wp_link_pages(
		[
			'before' => beans_open_markup( 'beans_post_content_navigation', 'p', [ 'class' => 'uk-text-bold' ] ) . beans_output( 'beans_post_content_navigation_text', __( 'Pages:', 'tm-beans' ) ),
...
function beans_post_meta_categories() {
	...
	beans_open_markup_e( 'beans_post_meta_categories', 'span', [ 'class' => 'uk-text-small uk-text-muted uk-clearfix' ] );
...
function beans_post_meta_tags() {
	...
	beans_open_markup_e( 'beans_post_meta_tags', 'span', [ 'class' => 'uk-text-small uk-text-muted uk-clearfix' ] );
...
function beans_post_navigation() {
	...
	beans_open_markup_e(
		...
		if ( $previous ) {
			beans_open_markup_e( 'beans_post_navigation_item[_previous]', 'li', [ 'class' => 'uk-pagination-previous' ] );
		...
		if ( $next ) {
			beans_open_markup_e( 'beans_post_navigation_item[_next]', 'li', [ 'class' => 'uk-pagination-next' ] );
...
function beans_posts_pagination() {
	...
	beans_open_markup_e(
		...
		foreach ( range( 1, (int) $wp_query->max_num_pages ) as $link ) {
			...
			// Integer.
			if ( $link === $current ) {
				beans_open_markup_e( 'beans_posts_pagination_item[_active]', 'li', [ 'class' => 'uk-active' ] );
...
function beans_no_post() {
	beans_open_markup_e( 'beans_post', 'article', [ 'class' => 'tm-no-article uk-article' . ( current_theme_supports( 'beans-default-styling' ) ? ' uk-panel-box' : null ) ] );

		beans_open_markup_e( 'beans_post_header', 'header' );

			beans_open_markup_e( 'beans_post_title', 'h1', [ 'class' => 'uk-article-title' ] );
		...
		beans_open_markup_e( 'beans_post_body', 'div' );
			...
			beans_open_markup_e( 'beans_post_content', 'div', [ 'class' => 'tm-article-content' ] );

				beans_open_markup_e( 'beans_no_post_article_content', 'p', [ 'class' => 'uk-alert uk-alert-warning' ] );
...
function beans_post_password_form() {
	...
	// Notice.
	$output = beans_open_markup( 'beans_password_form_notice', 'p', [ 'class' => 'uk-alert uk-alert-warning' ] );
...
function beans_post_gallery( $output, $attr, $instance ) {
	...
	$output = apply_filters( 'gallery_style', $gallery_div ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Used in function scope.
		...
		if ( 'none' === $atts['link'] ) {
			...
			$output .= beans_open_markup( "beans_post_gallery_item[_{$attachment_id}]", $atts['itemtag'], [ 'class' => 'gallery-item' ] );

				$output .= beans_open_markup( "beans_post_gallery_icon[_{$attachment_id}]", $atts['icontag'], [ 'class' => "gallery-icon {$orientation}" ] ); // Attributes are automatically escaped.
		...
		if ( $atts['captiontag'] && trim( $attachment->post_excerpt ) ) {
			$output .= beans_open_markup( "beans_post_gallery_caption[_{$attachment_id}]", $atts['captiontag'], [ 'class' => 'wp-caption-text gallery-caption' ] );
