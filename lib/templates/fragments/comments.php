...
function beans_comment_avatar() {
	...
	beans_open_markup_e( 'beans_comment_avatar', 'div', [ 'class' => 'uk-comment-avatar' ] );

		echo $avatar; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Echoes get_avatar().
...
function beans_comment_badges() {
	...
	if ( 'trackback' === $comment->comment_type ) {
		beans_open_markup_e( 'beans_trackback_badge', 'span', [ 'class' => 'uk-badge uk-margin-small-left' ] );
	...
	if ( 'pingback' === $comment->comment_type ) {
		beans_open_markup_e( 'beans_pingback_badge', 'span', [ 'class' => 'uk-badge uk-margin-small-left' ] );
	...
	if ( '0' === $comment->comment_approved ) {
		beans_open_markup_e( 'beans_moderation_badge', 'span', [ 'class' => 'uk-badge uk-margin-small-left uk-badge-warning' ] );
	...
	if ( user_can( $comment->user_id, 'moderate_comments' ) ) {
		beans_open_markup_e( 'beans_moderator_badge', 'span', [ 'class' => 'uk-badge uk-margin-small-left' ] );
...
function beans_comment_metadata() {
	beans_open_markup_e( 'beans_comment_meta', 'div', [ 'class' => 'uk-comment-meta' ] );
...
function beans_comment_links() {
	...
	beans_open_markup_e( 'beans_comment_links', 'ul', [ 'class' => 'tm-comment-links uk-subnav uk-subnav-line' ] );
...
function beans_comments_closed() {
	beans_open_markup_e( 'beans_comments_closed', 'p', [ 'class' => 'uk-alert uk-alert-warning uk-margin-bottom-remove' ] );
...
function beans_comments_navigation() {
	...
	beans_open_markup_e(
		...
		if ( get_previous_comments_link() ) {
			beans_open_markup_e( 'beans_comments_navigation_item[_previous]', 'li', [ 'class' => 'uk-pagination-previous' ] );
				...
				echo get_previous_comments_link( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Echoes HTML output.
		...
		if ( get_next_comments_link() ) {
			beans_open_markup_e( 'beans_comments_navigation_item[_next]', 'li', [ 'class' => 'uk-pagination-next' ] );
				...
				echo get_next_comments_link( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Echoes HTML output.
...
function beans_comment_form_divider() {
	beans_selfclose_markup_e( 'beans_comment_form_divider', 'hr', [ 'class' => 'uk-article-divider' ] );
}
...
function beans_comment_form() {
	$output = beans_open_markup( 'beans_comment_form_wrap', 'div', [ 'class' => 'uk-form tm-comment-form-wrap' ] );

		$output .= beans_render_function( 'comment_form', [ 'title_reply' => beans_output( 'beans_comment_form_title_text', __( 'Add a Comment', 'tm-beans' ) ) ] );

	$output .= beans_close_markup( 'beans_comment_form_wrap', 'div' );

	$submit = beans_open_markup(
		'beans_comment_form_submit',
		'button',
		[
	...
	echo preg_replace( '#<input[^>]+type="submit"[^>]+>#', $submit, $output ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped above or as an attribute.
}
...
function beans_comment_cancel_reply_link( $html, $link, $text ) {
	...
	$output = beans_open_markup(
		'beans_comment_cancel_reply_link',
		'a',
		[
			'rel'   => 'nofollow',
			'id'    => 'cancel-comment-reply-link',
			'class' => 'uk-button uk-button-small uk-button-danger uk-margin-small-right',
			// phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification -- Used to determine inline style.
...
function beans_comment_form_comment() {
	$output = beans_open_markup( 'beans_comment_form[_comment]', 'fieldset', [ 'class' => 'uk-margin-top' ] );
	...
	if ( beans_apply_filters( 'beans_comment_form_legend[_comment]', true ) ) {
		...
		$output .= beans_open_markup(
			'beans_comment_form_field[_comment]', 'textarea', [
				'id'       => 'comment',
				'class'    => 'uk-width-1-1',
				'name'     => 'comment',
				'required' => '',
...
function beans_comment_before_fields() {
	beans_open_markup_e( 'beans_comment_fields_wrap', 'div', [ 'class' => 'uk-width-medium-1-1' ] );

		beans_open_markup_e(
			'beans_comment_fields_inner_wrap', 'div', [
...
function beans_comment_form_fields( $fields ) {
	...
	if ( isset( $fields['author'] ) ) {
		...
		$author = beans_open_markup( 'beans_comment_form[_name]', 'div', [ 'class' => "uk-width-medium-1-$grid" ] );
		...
		if ( beans_apply_filters( 'beans_comment_form_legend[_name]', true ) ) {
			...
			$author .= beans_selfclose_markup(
				'beans_comment_form_field[_name]', 'input', [
					'id'       => 'author',
					'class'    => 'uk-width-1-1',
					'type'     => 'text',
	...
	if ( isset( $fields['email'] ) ) {
		$email = beans_open_markup( 'beans_comment_form[_email]', 'div', [ 'class' => "uk-width-medium-1-$grid" ] );
		...
		if ( beans_apply_filters( 'beans_comment_form_legend[_email]', true ) ) {
			...
			$email .= beans_selfclose_markup(
				'beans_comment_form_field[_email]', 'input', [
					'id'       => 'email',
					'class'    => 'uk-width-1-1',
					'type'     => 'text',
	...
	// Url.
	if ( isset( $fields['url'] ) ) {
		$url = beans_open_markup( 'beans_comment_form[_website]', 'div', [ 'class' => "uk-width-medium-1-$grid" ] );
		...
		if ( beans_apply_filters( 'beans_comment_form_legend[_url]', true ) ) {
			...
			$url .= beans_selfclose_markup(
				'beans_comment_form_field[_url]', 'input', [
					'id'    => 'url',
					'class' => 'uk-width-1-1',
					'type'  => 'text',
