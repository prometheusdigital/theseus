...
function beans_favicon() {
	...
	$url = file_exists( get_stylesheet_directory() . '/favicon.ico' ) ? get_stylesheet_directory_uri() . '/favicon.ico' : THESEUS_URL . 'favicon.ico';
...
function beans_header_image() {
	...
	if ( ! current_theme_supports( 'custom-header' ) || ! $header_image || empty( $header_image ) ) {
		return;
	}
