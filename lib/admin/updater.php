...
/**
 * Retrieve product data from the Theseus REST API.
 *
...
function beans_updater( $value ) {
	...
	// Query the Theseus REST API if the transient is expired.
	if ( empty( $data ) ) {
		$response = wp_remote_get( 'https://prometheus-digital.com/theseus/rest-api/' );
		...
		if ( is_wp_error( $response ) || isset( $data['error'] ) ) {
			set_site_transient( 'beans_updater', [ 'version' => $current_version ], 30 * MINUTE_IN_SECONDS );
			return $value;
		}
	...
	// Return data if Theseus is not up to date.
