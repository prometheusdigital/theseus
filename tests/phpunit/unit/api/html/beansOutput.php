...
class Tests_BeansOutput extends HTML_Test_Case {
	...
	public function test_should_return_filtered_output_after_firing_output_filter_hook() {
		...
		Monkey\Functions\expect( 'beans_apply_filters' )
			->once()
			->with( 'beans_archive_title_text_output', 'Theseus rocks!' )
		...
		// Run the tests.
		$this->assertSame( 'WooHoo, I fired!', beans_output( 'beans_archive_title_text', 'Theseus rocks!' ) );
	...
	public function test_should_return_output_when_not_in_html_dev_mode() {
		...
		$this->assertSame( 'Theseus rocks!', beans_output( 'beans_archive_title_text', 'Theseus rocks!' ) );
	}
...
EOB;
		$this->assertSame( $expected, beans_output( 'beans_archive_title_text', 'Theseus rocks!' ) );
	}
	...
	public function test_should_pass_additional_args_when_firing_filter_hook() {
		...
		Monkey\Functions\expect( 'beans_apply_filters' )
			->twice()
			->with( 'beans_breadcrumb_item_text_output', 'Theseus rocks!', 47, 'Hello' )
			->andReturnUsing( function( $hook, $output ) {
				return $output;
			} );
		...
		$this->assertSame(
			'Theseus rocks!',
			beans_output( 'beans_breadcrumb_item_text', 'Theseus rocks!', 47, 'Hello' )
		);

		// Check with HTML dev mode enabled.
		$expected = <<<EOB
<!-- open output: beans_breadcrumb_item_text -->Theseus rocks!<!-- close output: beans_breadcrumb_item_text -->
EOB;
		Monkey\Functions\expect( '_beans_is_html_dev_mode' )->once()->andReturn( true );
		$this->assertSame(
			$expected,
