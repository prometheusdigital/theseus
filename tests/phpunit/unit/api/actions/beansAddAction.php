...
class Tests_BeansAddAction extends Actions_Test_Case {
	...
	/**
	 * Test beans_add_action() should add the action in both Theseus and WordPress.
	 */
	public function test_should_add_action_in_beans_and_wordpress() {
		...
		foreach ( static::$test_actions as $beans_id => $action ) {
			...
			// Check that it was registered in Theseus and WordPress.
	...
	/**
	 * Test beans_add_action() should overwrite the action in both Theseus and WordPress.
	 *
	...
	public function test_should_overwrite_add_action_in_beans_and_wordpress() {
		...
		foreach ( static::$test_actions as $beans_id => $action ) {
			...
			// Now check that it's still registered in both Theseus and WordPress.
			$this->assertArrayHasKey( $beans_id, $_beans_registered_actions['added'] );
			$this->assertSame( $action, $_beans_registered_actions['added'][ $beans_id ] );
	...
	public function test_should_use_replaced_action_when_available() {
		...
		foreach ( static::$test_actions as $beans_id => $original_action ) {
			...
			Monkey\Functions\expect( '_beans_get_action' )
				...
				->with( $beans_id, 'replaced' )
				->ordered()
				->andReturn( $replaced_action ) // Simulates that the replaced action is registered in Theseus.
				->andAlsoExpectIt()
				->once()
				->with( $beans_id, 'removed' )
				->ordered()
	...
	public function test_should_return_false_when_removed() {
		...
		foreach ( static::$test_actions as $beans_id => $action ) {
			...
			Monkey\Functions\expect( '_beans_get_action' )
				...
				->with( $beans_id, 'removed' )
				->ordered()
				->andReturn( $empty_action )  // Simulates that the "removed" action is registered in Theseus.
				->andAlsoExpectIt()
				->with( $beans_id, 'modified' )
				->never();
			Monkey\Functions\expect( '_beans_set_action' )
			...
			// Check if the action is stored as "added" in Theseus.
			$this->assertArrayHasKey( $beans_id, $_beans_registered_actions['added'] );
			$this->assertSame( $action, $_beans_registered_actions['added'][ $beans_id ] );
	...
	public function test_should_merge_modified_action_parameters() {
		...
		foreach ( static::$test_actions as $beans_id => $original_action ) {
			...
			Monkey\Functions\expect( '_beans_get_action' )
				...
				->with( $beans_id, 'modified' )
				->ordered()
				->andReturn( $modified_action ); // Simulates that the "modified" action is registered in Theseus.
			Monkey\Functions\expect( '_beans_set_action' )
				->once()
