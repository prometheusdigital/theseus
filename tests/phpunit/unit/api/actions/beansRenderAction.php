...
class Tests_BeansRenderAction extends Test_Case {
	...
	public function test_should_return_after_calling_hook_with_no_subhook() {
		...
		$callback      = function() use ( $expected_args ) {
			...
			echo $args[1][0];  // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Not testing escaping functionality.
		};
		...
		$message = 'Calling the static method...and Theseus rocks!';
	...
	public function test_should_render_base_hook() {
		...
		$this->assertEquals( 'Theseus rocks!', _beans_render_action( 'beans_stub[_pre][_before]', 'Theseus rocks!' ) );
		...
		$this->assertEquals( 'Theseus rocks!', _beans_render_action( 'beans_stub[_subhook][_subsubhook]_after', 'Theseus rocks!' ) );
	}
