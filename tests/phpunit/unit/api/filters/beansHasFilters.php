 ...
 *
 * @package PrometheusDigital\Theseus\Tests\Unit\API\Filters
 *
 * @since   1.5.0
 */

namespace PrometheusDigital\Theseus\Tests\Unit\API\Filters;

use PrometheusDigital\Theseus\Tests\Unit\API\Filters\Includes\Filters_Test_Case;
...
class Tests_BeansHasFilters extends Filters_Test_Case {
	...
	public function test_should_return_priority_number_when_callback_registered() {
		...
		foreach ( static::$test_filters as $filter ) {
			...
			beans_add_filter( $filter['hook'], $filter['callback'] );

			$this->assertTrue( beans_has_filters( $filter['hook'], $filter['callback'] ) );

			remove_filter( $filter['hook'], $filter['callback'] );
		}
	}
}
