...
class Tests_BeansPathToUrl extends Test_Case {
	...
	/**
	 * Relative path to the Theseus directory starting from `wp-content`.
	 *
	 * @var string
	 */
	...
	protected function setUp() {
		...
		if ( ! $this->beans_relative_path ) {
			...
			$this->beans_relative_path       = wp_normalize_path(
				rtrim( str_replace( $abspath, '', THESEUS_THEME_DIR ), DIRECTORY_SEPARATOR )
			);
			$this->beans_tests_relative_path = wp_normalize_path(
				str_replace( $abspath, '', THESEUS_TESTS_DIR )
			);
		}
	}
	...
	public function test_should_bail_out_when_url() {
		...
		$this->assertSame( 'https://prometheus-digital.com/theseus', beans_path_to_url( 'https://prometheus-digital.com/theseus' ) );
	}
