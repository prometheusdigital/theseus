<?php
/**
 * Test Case for Theseus' Template API unit tests.
 *
 * @package PrometheusDigital\Theseus\Tests\Unit\API\Template\Includes
 *
 * @since   1.5.0
 */

namespace PrometheusDigital\Theseus\Tests\Unit\API\Template\Includes;
...
abstract class Template_Test_Case extends Test_Case {
	...
	protected function setUp() {
		...
		if ( ! defined( 'THESEUS_STRUCTURE_PATH' ) ) {
			define( 'THESEUS_STRUCTURE_PATH', vfsStream::url( 'templates/structure/' ) );
		}

		if ( ! defined( 'THESEUS_FRAGMENTS_PATH' ) ) {
			define( 'THESEUS_FRAGMENTS_PATH', vfsStream::url( 'templates/fragments/' ) );
		}
	}
	...
	private function set_up_virtual_filesystem() {
		...
		$file_structure                             = [
			'fragments' => [
				'branding.php'  => '<div class="tm-site-branding"><a href="http://example.com">Theseus Tests</a></div>',
