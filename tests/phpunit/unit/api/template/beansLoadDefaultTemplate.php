 ...
 *
 * @package PrometheusDigital\Theseus\Tests\Unit\API\Template
 *
 * @since   1.5.0
 */

namespace PrometheusDigital\Theseus\Tests\Unit\API\Template;

use PrometheusDigital\Theseus\Tests\Unit\API\Template\Includes\Template_Test_Case;
...
class Tests_BeansLoadDefaultTemplate extends Template_Test_Case {
	...
	public function test_should_return_false_when_template_does_not_exist() {
		...
		$this->assertFileNotExists( THESEUS_STRUCTURE_PATH . 'does-not-exist.php' );
		$this->assertFalse( beans_load_default_template( $file ) );

		$this->assertFileNotExists( THESEUS_STRUCTURE_PATH . basename( __FILE__ ) );
	 ...
	 * basename is a Theseus' structure.
	 */
	public function test_should_load_default_structure_when_given_file_does_not_exist_but_basename_is_beans_structure() {
		...
		// Check that 'content.php' does exist in Theseus.
		$this->assertFileExists( THESEUS_STRUCTURE_PATH . basename( $file ) );
		...
		// Check that 'header.php' does exist in Theseus.
		$this->assertFileExists( THESEUS_STRUCTURE_PATH . basename( $file ) );
