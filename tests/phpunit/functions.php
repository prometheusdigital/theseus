<?php
/**
 * Theseus' Test Suites' common functionality.
 *
 * @package PrometheusDigital\Theseus\Tests
 *
 * @since   1.5.0
 */

namespace PrometheusDigital\Theseus\Tests;
...
function init_test_suite( $test_suite ) {
	...
	// Load the files.
	$beans_root_dir = rtrim( THESEUS_THEME_DIR, DIRECTORY_SEPARATOR );
	require_once $beans_root_dir . '/vendor/autoload.php';
	require_once __DIR__ . '/test-case-trait.php';

	// Load Patchwork before everything else in order to allow us to redefine WordPress and Theseus functions.
...
function check_readiness() {
	...
	if ( version_compare( phpversion(), '5.6.0', '<' ) ) {
		trigger_error( 'Theseus Unit Tests require PHP 5.6 or higher.', E_USER_ERROR ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error -- Valid use case for our testing suite.
	}
...
function init_constants( $test_suite_folder ) {
	define( 'THESEUS_TESTS_DIR', __DIR__ . DIRECTORY_SEPARATOR . $test_suite_folder );

	$beans_root_dir = dirname( dirname( __DIR__ ) );
	if ( 'unit' === $test_suite_folder ) {
		$beans_root_dir .= DIRECTORY_SEPARATOR;
	}
	define( 'THESEUS_THEME_DIR', $beans_root_dir );
}

/**
 * Resets Theseus for the test suite.
 *
 * @since 1.5.0
 *
 * @return void
