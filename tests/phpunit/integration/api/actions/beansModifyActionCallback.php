 ...
 *
 * @package PrometheusDigital\Theseus\Tests\Integration\API\Actions
 *
 * @since   1.5.0
 */

namespace PrometheusDigital\Theseus\Tests\Integration\API\Actions;

use PrometheusDigital\Theseus\Tests\Integration\API\Actions\Includes\Actions_Test_Case;
...
class Tests_BeansModifyActionCallback extends Actions_Test_Case {
	...
	public function test_should_not_modify_when_invalid_callback() {
		...
		foreach ( static::$test_actions as $beans_id => $original_action ) {
			...
			foreach ( $callbacks as $callback ) {
				...
				// Check that the callback did not get stored as "modified" in Theseus.
	...
	/**
	 * Test beans_modify_action_callback() should register with Theseus as "modified", but not add the action.
	 */
	public function test_should_register_as_modified_but_not_add_action() {
		...
		foreach ( static::$test_actions as $beans_id => $action ) {
			...
			// Check that it did register as "modified" in Theseus.
	...
	public function test_should_modify_the_action_callback() {
		...
		foreach ( static::$test_actions as $beans_id => $original_action ) {
			// Check that the original action is registered in WordPress and in Theseus as "added".
			...
			// Check that the modified action is registered as "modified" in Theseus.
